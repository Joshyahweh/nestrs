...
# nestrs-mcp = { version = "1.0", features = ["http", "admin", "authz", "elicitation"] }
```
...
<Warning>
The wrapper declares **no tools** — its `#[tool_router]` impl block is empty, so a `tools/list` against the stock binary returns `[]`. The tools live on the four area handlers below; serve one of those (or your own) from your binary. See [Embedding in another binary](#embedding-in-another-binary).
</Warning>
