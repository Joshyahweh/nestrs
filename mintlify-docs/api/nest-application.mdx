...
NestJS `MiddlewareConsumer` analogue: apply a middleware function to selected path prefixes (`for_routes`) with optional `exclude`. Matching uses path-prefix `starts_with`, not Nest's glob `ab*cd` DSL. These layers sit **inside** `use_global_layer` (your global layers wrap them).

```rust
pub fn configure_middleware(self, consumer: MiddlewareConsumer) -> Self
```
...
Installs per-route throttling (`#[throttle(n, "per")]` / `#[skip_throttle]`) plus a 429 layer with `Retry-After` and `X-RateLimit-Remaining`. Requires the `throttler` feature. `ThrottlerOptions` has no builder — construct the struct. `global: None` (default) throttles only decorated routes.

```rust
#[cfg(feature = "throttler")]
pub fn use_throttler(self, options: ThrottlerOptions) -> Self
```
...
See the [throttling guide](/guides/throttling) for Redis backends, skippers, and key generators.
...
Enables Redis-backed server-side sessions via `RedisSessionStore` (`SET`/`GET`/`DEL` with `PX` TTL). Implies `use_cookies`. Requires the `session-redis` feature. If both memory and Redis are set, Redis wins and a warning is logged. The session cookie is `Secure` when the process is in production (`NESTRS_ENV` / `APP_ENV` / `RUST_ENV`). `Debug` redacts `user:pass@`; session ids are never logged.

```rust
#[cfg(feature = "session-redis")]
pub fn use_session_redis(self, url: impl Into<String>) -> Self
```
...
Assigns a stable `x-request-id` to each request and echoes it on the response. When the client supplies an id it is honored — but only if it is plain identifier-ish ASCII (`[A-Za-z0-9._-]`, non-empty, at most 128 bytes, UUID/ULID/hex style); anything else is replaced with a fresh UUID, so unparseable traffic can never forge correlation ids or smuggle log-forging payloads through the request-id channel.
...
<AccordionGroup>
  ...
  <Accordion title="enable_production_errors — sanitize 5xx bodies">
    Strips internal detail from JSON 5xx bodies: sets `message` to a generic string and removes the `errors` field. **On by default whenever the runtime environment is production** (first non-empty of `NESTRS_ENV`, `APP_ENV`, or `RUST_ENV` equals `production` or `prod`) — no builder call needed. `enable_production_errors()` forces sanitization on in any environment (e.g. `NESTRS_ENV=staging`); `enable_production_errors_from_env()` re-reads the environment explicitly; `disable_production_errors()` opts out for internal services behind a trusted boundary.
