---
title: "Generate code with nestrs-cli generate"
description: "Use nestrs-cli generate (or nestrs-cli g) to scaffold services, controllers, modules, DTOs, guards, pipes, filters, interceptors, and full CRUD resources."
---

`nestrs-cli generate` (aliased as `nestrs-cli g`) creates individual source files or full multi-file resource scaffolds from built-in templates. Every generated file uses real nestrs macros and compiles out of the box. For resource generators, the CLI also automatically wires the new module into your parent `main.rs`, `lib.rs`, or `mod.rs`.

## Syntax

```bash
nestrs-cli g <kind> <name> [flags]
nestrs-cli generate <kind> <name> [flags]
```
...
nestrs-cli g resource users

# Explicit transport
nestrs-cli g resource users --transport rest --path src

# Dry run first
nestrs-cli g resource users --transport graphql --path src --dry-run
```
...
<AccordionGroup>
  ...
  <Accordion title="resolver (standalone)">
    The standalone `resolver` generator produces a bare stub. For a full GraphQL CRUD resolver, use `nestrs-cli g resource users --transport graphql`.
  ...
  <Accordion title="gateway (standalone)">
    The standalone `gateway` generator produces a bare stub. For a full WebSocket gateway, use `nestrs-cli g resource users --transport ws`.
  ...
  <Accordion title="transport (standalone)">
    The standalone `transport` generator produces a bare stub. For full message patterns, use `nestrs-cli g resource users --transport microservice` or `--transport grpc`.
...
nestrs-cli g resource invoices --transport rest --path src
```

```bash generate a WebSocket resource
nestrs-cli g resource notifications --transport ws --path src
```

```bash generate individual files
nestrs-cli g service payments --path src
nestrs-cli g guard auth --path src
nestrs-cli g dto create-payment --path src
```

```bash dry run before committing
nestrs-cli g resource orders --transport grpc --path src --dry-run
```
