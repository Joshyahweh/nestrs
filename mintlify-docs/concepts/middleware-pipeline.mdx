...
## Path-scoped middleware — `MiddlewareConsumer`

Nest's `configure(consumer: MiddlewareConsumer)` analogue is `NestApplication::configure_middleware`. Use `apply_fn` then `for_routes` / `exclude` to scope an Axum middleware function to path prefixes (`starts_with`; no glob `ab*cd` DSL). Non-matching paths skip the function.
...
These layers sit **inside** `use_global_layer` (your global layers wrap them). For process-wide Tower layers, keep using `use_global_layer`.
