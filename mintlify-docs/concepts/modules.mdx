...
## Namespaced configuration (`#[config(namespace)]`)
...
The macro emits the required `serde::Deserialize` + `validator::Validate` derives and the `nestrs::ConfigNamespace` marker impl, so the struct itself needs no derives — but the attribute must come **before** any derives you add yourself. Values are read from `NESTRS_DB__HOST` / `NESTRS_DB__PORT` (prefix `NESTRS_`, double-underscore separator between namespace and key). The overlay merges dotenvy cascade files (`.env`, `.env.<NESTRS_ENV>`, in non-production only) with the process environment last, so config resolution is testable without mutating process env.

Register typed namespaces as modules:

```rust
use nestrs::config::ConfigModule;

let dm = ConfigModule::for_root(vec![nestrs::config::Config::register::<DatabaseConfig>()]);

#[module(imports = [dm], providers = [DbService])]
struct AppModule;
```

Inside any `#[injectable]`, resolve the typed config via `ConfigService`:

```rust
#[injectable]
struct DbService {
    cfg: std::sync::Arc<nestrs::config::ConfigService>,
}
...
Validation runs on load: a struct whose field fails its `validator` attribute surfaces a `ConfigError` at startup (fail-fast) rather than a garbled value deep in a request handler. For a single non-namespaced struct, `nestrs::config::load_config::<T>()` reads flat env vars (`HOST`, `PORT`) with the same dotenvy cascade + validation behavior.
