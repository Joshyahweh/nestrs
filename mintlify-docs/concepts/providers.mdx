...
impl Injectable for DatabaseService {
    ...
    async fn on_before_application_shutdown(&self) {
        // NestJS `beforeApplicationShutdown`: flush caches while dependents are still alive.
    }
...
|---|---|---|
| `register_use_value::<T>(Arc<T>)` | `useValue` | Pre-built singleton; lifecycle hooks are **not** run — any `T` is accepted, so there is no hook impl to call |
| `register_use_value_with_lifecycle::<T>(Arc<T>)` | `useValue` + `OnModuleInit` & friends | Requires `T: ProviderLifecycle`; the framework drives `T`'s five hooks like an `Injectable` |
| `register_use_factory::<T>(scope, \|reg\| Arc<T>)` | `useFactory` | Sync closure; call `reg.get()` for dependencies; lifecycle hooks are **not** run |
| `register_use_factory_with_lifecycle::<T>(scope, \|reg\| Arc<T>)` | `useFactory` + lifecycle | Requires `T: ProviderLifecycle`; hooks run while `T` is singleton-scoped |
