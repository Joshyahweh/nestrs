...
- **`nestrs-sea-orm` (recommended ORM)** — SeaORM `Repo`, ambient transactions, `RowAuthz` / `AbilityAuthz`. See [adapters/sea-orm](/adapters/sea-orm).
...
For a TypeORM/Sequelize-style Active Record layer, prefer [`nestrs-sea-orm`](/adapters/sea-orm).
...
```toml
[dependencies]
nestrs = { version = "1.5.0", features = ["database-sqlx"] }
...
nestrs-cli db migrate run          # apply pending
nestrs-cli db migrate info        # list applied/pending
nestrs-cli db migrate revert      # walk down (--target-version 0 for full)

nestrs-cli db seed --bin seed_posts        # cargo run --bin seed_posts
nestrs-cli db seed --seed-file dev.sql      # transactional raw SQL
```
...
See the [CLI reference](/cli/overview) for the full flag table.
...
```bash
nestrs-cli db --backend prisma migrate run   # pass-through to npx prisma
...
`MongoModule` wraps the official `mongodb` driver. Call `for_root` (or
`for_root_async` for vault / env factories) with a connection URI before
`NestFactory::create`, then inject `MongoService` to access databases and
collections. `MongoService::model::<T>()` is the NestJS `@InjectModel`
analogue over `MongoRepository::for_feature`.
...
// Async factory (vault / ConfigService) — must complete before create:
// MongoModule::for_root_async(|| async {
//     Ok(nestrs::mongo::MongoOptions::new(std::env::var("MONGO_URI")?))
// }).await?;
...
`MongoService::model::<T>()` is the NestJS `@InjectModel` analogue: it
returns `MongoRepository<T>` for a type that implements `Document`
(`#[derive(Document)]`) after `MongoModule::for_feature("db_name")`.
See the [MongoDB recipe](/recipes/mongodb).
