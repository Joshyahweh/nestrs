...
<CardGroup cols={2}>
  ...
  <Card title="Adapter crates" icon="plug" href="/ecosystem/adapters">
...
<Note>
`nestrs-prisma`, `nestrs-storage`, and the [adapter crates](/ecosystem/adapters) (`nestrs-socketio`, `nestrs-lambda`, `nestrs-passport`, `nestrs-better-auth`, `nestrs-bullmq`, `nestrs-saml`, `nestrs-ldap`, `nestrs-sea-orm`) are separate crates, not feature flags on `nestrs`. Add `nestrs-prisma` with one of `sqlx-postgres`, `sqlx-mysql`, or `sqlx-sqlite`. Add `nestrs-storage` with one of `local`, `s3`, `gcs`, `azure`, or `all`.
</Note>
