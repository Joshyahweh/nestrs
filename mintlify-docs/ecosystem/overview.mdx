...
<CardGroup cols={2}>
  ...
  <Card title="QueuesModule" icon="list" href="/ecosystem/scheduling">
    In-process baseline queue (Bull-style API) under `queues`. For multi-instance
    production, enable `queues-redis` — durable Redis lists (`LPUSH` / `BRPOP`),
    not the BullMQ wire protocol. Declare processors with `#[queue_processor("NAME")]`.
  </Card>
  ...
  <Card title="Adapter crates" icon="plug" href="/adapters/overview">
    Socket.IO, Lambda, Passport, Better Auth, BullMQ, SAML, LDAP, and SeaORM — companion
    crates for NestJS surfaces that do not fold into Axum.
  </Card>
</CardGroup>
...
| `queues-redis` | Durable Redis lists (`LPUSH` / `BRPOP`) for `QueuesModule`. Not the BullMQ protocol. |
| `session` | In-memory `tower_sessions` (`use_session_memory`) |
| `session-redis` | Redis-backed sessions (`use_session_redis`) |
...
| `http-client` | HTTP client utilities (`HttpModule` / `HttpService`) |
| `sse` | Server-Sent Events (`SseResponse`, `serialize_to_event`) |
| `throttler` | Per-route rate limits (`ThrottlerModule`, `ThrottlerGuard`) |
...
<Note>
`nestrs-prisma`, `nestrs-storage`, and the [adapter crates](/adapters/overview) (`nestrs-socketio`, `nestrs-lambda`, `nestrs-auth-strategy`, `nestrs-better-auth`, `nestrs-bullmq`, `nestrs-saml`, `nestrs-ldap`, `nestrs-sea-orm`) are separate crates, not feature flags on `nestrs`. Add `nestrs-prisma` with one of `sqlx-postgres`, `sqlx-mysql`, or `sqlx-sqlite`. Add `nestrs-storage` with one of `local`, `s3`, `gcs`, `azure`, or `all`.
</Note>
...
# Cargo.toml
[dependencies]
nestrs = { version = "1.3.0", features = ["cache", "schedule"] }
