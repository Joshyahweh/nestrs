...
<Note>
`ScheduleModule` is designed for app-local tasks — work that belongs in the same process as your HTTP server. It is not a distributed scheduler. For durable, multi-instance job queues, enable **`queues-redis`** on `QueuesModule` (Redis `LPUSH` / `BRPOP`). That backend is Bull-style (lists + JSON envelopes), not the BullMQ wire protocol.
</Note>
...
```toml
[dependencies]
nestrs = { version = "1.5.0", features = ["schedule"] }
