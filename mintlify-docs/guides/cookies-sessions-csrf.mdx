...
description: "Opt-in browser-facing state with `use_cookies`, `use_session_memory`, and `use_csrf_protection` — the @nestjs/csrf analogue built on tower-cookies + tower-sessions."
---
...
server-side sessions via tower-sessions), and **`csrf`** (double-submit
CSRF protection on unsafe methods). All three are opt-in — API-only
services don't pay for them, browser-facing apps get the layers on the
exact routes they need.
...
# Cargo.toml
[dependencies]
nestrs = { version = "1.0.0", features = ["cookies", "session", "csrf"] }
```
...
values, the store handles serialization. `MemoryStore` is the right
choice for single-process dev and tests; for multi-process prod, swap
to `tower_sessions::MysqlStore` / `RedisStore` / `CachingStore`
(whatever `tower-sessions` ships for your backing store) and wire it
in place of the `MemoryStore::default()` the builder installs.
...
> nestrs security: cookies or in-memory sessions are enabled, but CSRF
