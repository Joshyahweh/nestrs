...
```toml
[dependencies]
nestrs = { version = "1.0.0", features = ["graphql-federation-gateway"] }
```
...
Each subgraph contributes a federation-shaped SDL. Export one from an async-graphql `Schema` with `SDLExportOptions::default().federation()`:
