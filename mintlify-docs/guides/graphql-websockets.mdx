...
```toml
[dependencies]
nestrs = { version = "1.3.0", features = ["graphql-federation-gateway"] }
```
...
`nestrs-ws` speaks RFC 6455 JSON events. For the NestJS `@nestjs/platform-socket.io` protocol, add the companion crate:
...
io.ns("/", |s| async move { /* socketioxide handlers */ });
let router = merge_socketio(NestFactory::create::<AppModule>().into_router(), layer);
```
