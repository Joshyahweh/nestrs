...
As a library dependency, the same features apply — `nestrs-mcp = { version = "1.2.0", features = ["admin"] }` etc. The full feature list (`stdio`, `http`, `admin`, `authz`, `authz-row-level`, `elicitation`) is on the [API reference](/api/crates/nestrs-mcp).
...
```toml
nestrs-mcp = { version = "1.2.0", features = ["authz"] }
```
