...
## Transport hardening (TCP)

The TCP transport applies fixed resource bounds so a misbehaving or hostile peer cannot exhaust server memory:
...
| Client request timeout | 30 s | A `send` call to an unresponsive peer errors instead of hanging forever. |
| Max concurrent connections | 1024 | Enforced with a connection semaphore; connections past the cap are refused and the drop is logged (visible in `tracing`). |

These are protocol-level defaults, not configurable knobs — size large payloads differently (chunk, or move to Kafka/NATS for big messages) rather than trying to raise them.

## Broker failure semantics

What happens when a message cannot be processed is transport-specific:

- **RabbitMQ** — malformed (undeserializable) messages are `basic_nack`ed with **requeue disabled**: they are dropped, not redelivered. A handler **panic** is caught (`catch_unwind`); the error is published to the reply queue for RPC patterns and the message is nacked without requeue. Poison messages therefore never wedge the consumer's prefetch slots — a crashing handler cannot stall the whole work queue.
- **Kafka** — the listener keeps its position **in memory** (rskafka has no offset-commit API). Restarts follow `KafkaConsumerStart::Latest` (default, skip retained history) or `Earliest` (full replay). Delivery across restarts is at-most-once; make handlers idempotent.
- **NATS core** — messages are ephemeral; an offline subscriber misses them. Use JetStream for integration events you must not lose.
