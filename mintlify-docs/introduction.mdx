...
<Note>
**nestrs is 1.2.** The public API follows semantic versioning — breaking changes only ship in a new major version — and the public API surface of the core crates is snapshot-tested in CI, so accidental breaking changes are caught before a release lands. 1.2 adds `#[intersection_type]`, Redis sessions (`session-redis`), Mongo `for_root_async` / `MongoService::model`, live `nestrs-cli repl live`, and MCP tool dispatch.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="DTO validation" icon="shield-check" href="/guides/validation">
    `#[dto]` + `#[IsEmail]`, `#[Length]`, mapped types (`#[partial_type]`, `#[omit_type]`, `#[pick_type]`, `#[intersection_type]`), powered by the `validator` crate.
  </Card>
  ...
  <Card title="CLI scaffolding" icon="terminal" href="/cli/overview">
    `nestrs-cli new` and `nestrs-cli generate` create projects and resource skeletons in seconds.
  </Card>
</CardGroup>
...
| [`nestrs-throttle`](https://crates.io/crates/nestrs-throttle) | Per-route rate limits (`ThrottlerGuard`; umbrella feature `throttler`) |
| [`nestrs-health`](https://crates.io/crates/nestrs-health) | Liveness / readiness indicators |
| [`nestrs-security`](https://crates.io/crates/nestrs-security) | Auth helpers, helmet headers, CSRF |
| [`nestrs-oauth2`](https://crates.io/crates/nestrs-oauth2) | OAuth2 client, resource server, `OAuth2Guard` |
| [`nestrs-socketio`](https://crates.io/crates/nestrs-socketio) | Socket.IO via socketioxide |
| [`nestrs-lambda`](https://crates.io/crates/nestrs-lambda) | AWS Lambda / API Gateway (`listen_lambda`) |
| [`nestrs-passport`](https://crates.io/crates/nestrs-passport) | Passport-style `JwtStrategy` / `LocalBasicStrategy` |
| [`nestrs-better-auth`](https://crates.io/crates/nestrs-better-auth) | Better Auth router + session-cookie guard |
| [`nestrs-bullmq`](https://crates.io/crates/nestrs-bullmq) | BullMQ Redis key-layout producer |
| [`nestrs-saml`](https://crates.io/crates/nestrs-saml) / [`nestrs-ldap`](https://crates.io/crates/nestrs-ldap) | SAML SP redirect and LDAP simple bind |
| [`nestrs-sea-orm`](https://crates.io/crates/nestrs-sea-orm) | SeaORM `for_root_async` (TypeORM / Sequelize analogue) |
| [`nestrs-scaffold`](https://crates.io/crates/nestrs-scaffold) | CLI — install with `cargo install nestrs-scaffold`, binary name `nestrs-cli` |
