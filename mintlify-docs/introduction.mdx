...
| [`nestrs-socketio`](/adapters/socketio) | Socket.IO via socketioxide |
| [`nestrs-lambda`](/adapters/lambda) | AWS Lambda / API Gateway (`listen_lambda`) |
| [`nestrs-auth-strategy`](/adapters/auth-strategy) | Passport-style `JwtStrategy` / `LocalBasicStrategy` |
| [`nestrs-better-auth`](/adapters/better-auth) | Better Auth router + session-cookie guard |
| [`nestrs-bullmq`](/adapters/bullmq) | BullMQ Redis key-layout producer |
| [`nestrs-saml`](/adapters/saml) / [`nestrs-ldap`](/adapters/ldap) | SAML SP redirect and LDAP simple bind |
| [`nestrs-sea-orm`](/adapters/sea-orm) | SeaORM `for_root_async` (TypeORM / Sequelize analogue) |
