...
| `@nestjs/platform-socket.io` | `nestrs-socketio` | socketioxide layer on the Axum router |
| `@nestjs/platform-fastify` | — | Out of scope (ADR-0001, Axum-only) |
| AWS Lambda / serverless | `nestrs-lambda` (`listen_lambda`) | API Gateway HTTP + REST |
| `@nestjs/passport` | `nestrs-passport` | `JwtStrategy` / `LocalBasicStrategy` |
| nestjs-better-auth | `nestrs-better-auth` | Mount the better-auth.rs router + session-cookie guard |
| `@nestjs/bullmq` | `nestrs-bullmq` | Redis key-layout producer; Node workers can consume |
| `passport-saml` / `passport-ldap` | `nestrs-saml` / `nestrs-ldap` | SP redirect + simple bind; assertion crypto stays in the app |
| TypeORM / Sequelize | `nestrs-sea-orm` | `SeaOrmModule::for_root_async`; drizzle stays carved out |
