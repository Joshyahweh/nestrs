...
| `PartialType` / `OmitType` / `PickType` / `IntersectionType` | `#[partial_type]` / `#[omit_type(...)]` / `#[pick_type(...)]` / `#[intersection_type]` | Intersection flattens parent DTOs; parents need `#[dto(allow_unknown_fields)]` |
| `@InjectModel(User)` | `MongoService::model::<User>()` | After `MongoModule::for_feature`; also `MongoRepository::for_feature` |
| `MongooseModule.forRootAsync(...)` | `MongoModule::for_root_async(factory).await?` | Must complete before `NestFactory::create` |
| Redis session store | `.use_session_redis(url)` | Feature `session-redis`; Redis wins if memory is also set |
| `MiddlewareConsumer` `apply` / `forRoutes` / `exclude` | `MiddlewareConsumer` + `.configure_middleware(...)` | Path prefixes only; no glob `ab*cd` DSL |
| `@nestjs/platform-socket.io` | [`nestrs-socketio`](/adapters/socketio) | socketioxide layer on the Axum router |
| `@nestjs/platform-fastify` | — | Out of scope (ADR-0001, Axum-only) |
| AWS Lambda / serverless | [`nestrs-lambda`](/adapters/lambda) (`listen_lambda`) | API Gateway HTTP + REST |
| `@nestjs/passport` | [`nestrs-auth-strategy`](/adapters/auth-strategy) | `JwtStrategy` / `LocalBasicStrategy` |
| nestjs-better-auth | [`nestrs-better-auth`](/adapters/better-auth) | Mount the better-auth.rs router + session-cookie guard |
| `@nestjs/bullmq` | [`nestrs-bullmq`](/adapters/bullmq) | Redis key-layout producer; Node workers can consume |
| `passport-saml` / `passport-ldap` | [`nestrs-saml`](/adapters/saml) / [`nestrs-ldap`](/adapters/ldap) | SP redirect + simple bind; assertion crypto stays in the app |
| TypeORM / Sequelize | [`nestrs-sea-orm`](/adapters/sea-orm) | `SeaOrmModule::for_root_async`; drizzle stays carved out |
...
- **Express / Fastify HTTP engines** — Axum only (ADR-0001). Use `nestrs-lambda` for AWS Lambda.
- **Nest CLI monorepo / plugin system** — Cargo workspaces.
- **VS Code Nest generator extension** — tracked as [#39](https://github.com/Joshyahweh/nestrs/issues/39); MCP already covers Copilot.
</Warning>
