...
<Warning>
**Not a workspace member in 1.2.0.** Upstream `drizzle-orm 0.36` is not
...
workspace `members` until then. Use `database-sqlx` or `nestrs-prisma` for
production SQL.
</Warning>
...
```toml
[dependencies]
nestrs-drizzle = { version = "1.2.0", features = ["postgres"] }
