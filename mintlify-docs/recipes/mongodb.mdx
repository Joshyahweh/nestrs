...
```toml
[dependencies]
nestrs = { version = "1.1", features = ["mongo"] }
nestrs-mongodb = "1.1"
...
```rust
use nestrs::prelude::*;
use nestrs::mongo::{MongoModule, MongoService};
...
| `MongoService` | Injectable handle: `client()`, `database(name)`, `default_database()`, `ping()`, `list_databases()`. |
