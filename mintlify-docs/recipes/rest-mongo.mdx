...
For vault / env lookups, `MongoModule::for_root_async` must complete
before `NestFactory::create`. Typed repositories resolve with
`MongoService::model::<T>()` (`@InjectModel`) after
