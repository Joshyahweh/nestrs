...
### B) From your own app (crate consumers on `nestrs-prisma = "1.0.0"`)
...
```bash
mkdir -p examples
curl -fsSL "https://raw.githubusercontent.com/Joshyahweh/nestrs/v1.0.0/nestrs-prisma/examples/quickstart.rs" -o examples/quickstart.rs
```
...
cargo download nestrs-prisma==1.0.0 --extract
mkdir -p examples
cp nestrs-prisma-1.0.0/examples/quickstart.rs examples/quickstart.rs
```
