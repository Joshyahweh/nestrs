...
pub trait WsCanActivate: Default + Send + Sync + 'static {
    ...
    /// `#[ws_routes]` (and mounted by `#[ws_gateway]` via
    /// [`ws_route_with_registry`]) calls this **per message** — a guard
    /// that only implements `Default` keeps working unchanged.
...
pub trait WsPipeTransform: Default + Send + Sync + 'static {
    /// Build the pipe instance for a message; see [`WsCanActivate::resolve`].
    fn resolve(_registry: &nestrs_core::ProviderRegistry) -> Self
    where
        Self: Sized,
    {
...
/// Registry-aware gateway dispatch, generated by `#[ws_routes]` next to the
/// plain [`WsGateway`] impl.
///
/// Same event dispatch, but `#[use_ws_guards]` / `#[use_ws_pipes]` /
/// `#[use_ws_interceptors]` instances are built through
/// [`WsCanActivate::resolve`] / [`WsPipeTransform::resolve`] /
/// [`WsIncomingInterceptor::resolve`] with the app's registry — so guards
/// with injected dependencies receive them instead of a silently-empty
/// `Default`. `#[ws_gateway]` mounts gateways via
/// [`ws_route_with_registry`], which wraps them in
/// [`__NestrsWsRegistryGateway`] so `serve_socket`'s plain
/// [`WsGateway::on_message`] call transparently reaches this trait.
///
/// The plain [`WsGateway`] impl keeps `Default` construction and remains
/// valid for hand-written gateways mounted via [`ws_route`].
#[async_trait::async_trait]
pub trait RegistryAwareWsGateway: WsGateway {
    /// Registry-aware counterpart of [`WsGateway::on_message`].
...
/// `#[doc(hidden)]` wrapper mounted by [`ws_route_with_registry`]: carries
/// the app's registry next to the DI-resolved gateway so the shared
/// runtime — which dispatches plain [`WsGateway`] — transparently reaches
/// the registry-aware methods generated by `#[ws_routes]`.
#[doc(hidden)]
pub struct __NestrsWsRegistryGateway<G> {
    pub(crate) inner: Arc<G>,
    pub(crate) registry: nestrs_core::ProviderRegistry,
}

#[async_trait::async_trait]
impl<G: RegistryAwareWsGateway> WsGateway for __NestrsWsRegistryGateway<G> {
    async fn on_message(&self, client: WsClient, event: &str, payload: serde_json::Value) {
        self.inner
            .on_message_with_registry(client, event, payload, &self.registry)
            .await
    }

    fn message_guards(&self) -> WsGuardChain {
        self.inner.message_guards()
    }
}
...
/// Registry-aware variant of [`ws_route`]: mounts `gateway` wrapped so
/// `#[use_ws_guards]` / `#[use_ws_pipes]` / `#[use_ws_interceptors]`
/// resolve through the app's registry (DI-backed guards) instead of
/// `Default::default()`. This is what the generated `#[ws_gateway]`
/// controller registration calls; hand-written gateways that don't need
/// registry resolution keep using [`ws_route`] (or the security/guard
/// variants).
pub fn ws_route_with_registry<G>(
    gateway: Arc<G>,
    registry: nestrs_core::ProviderRegistry,
) -> axum::routing::MethodRouter
where
    G: RegistryAwareWsGateway,
{
    ws_route(Arc::new(__NestrsWsRegistryGateway {
        inner: gateway,
        registry,
    }))
}
